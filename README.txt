...
Плагины размещаются в папке `plugins`. При старте приложения все файлы
с расширением `.py` из этой директории автоматически загружаются.
В проекте есть пример `plugins/sample_plugin.py`, который выводит
сообщения в журнал при активации и деактивации. Также доступен
плагин `plugins/deepseek_chat_plugin.py`, обеспечивающий диалог
с моделью DeepSeek. Для работы последнего задайте переменную
окружения `DEEPSEEK_API_KEY`.

## Создание исполняемого файла

Для сборки самостоятельного исполняемого файла используется
[PyInstaller](https://pyinstaller.org/). В репозитории уже присутствует
конфигурационный файл `app_new_ui.spec`.

```bash
pip install pyinstaller
pyinstaller app_new_ui.spec
```

Готовый файл появится в директории `dist/app_new_ui/` и
может быть перенесён на другой компьютер без установки Python.
