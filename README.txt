...
Для демонстрации клиент-серверного взаимодействия в папке `server` добавлен простой gRPC сервер на Go.
Он предоставляет методы `Ping` и `ListServers`.
...
Пример клиента на Python расположен в файле `grpc_client.py`:
