...
Плагин `ChatAIPlugin` предоставляет простое окно чата с ИИ. Для работы требуется ключ API OpenAI. Установите переменную окружения `OPENAI_API_KEY` перед запуском приложения:

```bash
export OPENAI_API_KEY=your_key_here
```

После загрузки плагин добавит пункт **AI Chat** в меню "Инструменты".
