...
4.  **Установите переменную `OPENAI_API_KEY`:**
    Для работы вкладки «Чат» требуется ключ OpenAI.
    ```bash
...
На отдельной вкладке интерфейса можно вести диалог с моделью OpenAI. История
сообщений отображается в большом поле, а новые сообщения вводятся в строку
внизу. Чтобы использовать эту функцию, убедитесь, что переменная
